...
import psychologyData from '@/data/psychology.json'; // Import new JSON file

const TradingPsychology = () => {
  const [currentTab, setCurrentTab] = useState('Lessons'); // Manage active tab
  const [currentSection, setCurrentSection] = useState(
    psychologyData.sections[0]?.sectionTitle || '' // Default section
  );
  ...
  const generateQuizQuestions = () => {
    const questions = [
      ...psychologyData.sections.flatMap((section) =>
        section.content.topics.flatMap((topic) => [
          {
            ...
            explanation: `The concept of "${topic.topicTitle}" revolves around: ${topic.content}. Understanding this is crucial for effective trading psychology.`,
          },
          {
            ...
            explanation: `The key takeaway is to implement "${topic.topicTitle}" properly to improve psychological discipline in trading.`,
          },
        ])
      ),
    ];
    ...
    <Box sx={{ py: 4, px: 2, bgcolor: '#121212', color: '#D1C382', minHeight: '100vh' }}>
      ...
      {currentTab === 'Lessons' ? (
        <>
          ...
          >
            {psychologyData.sections.map((section) => (
